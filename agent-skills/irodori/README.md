# irodori スキル群 — 商品リサーチ・証拠整理・ランキング設計(再現性強化版)
...
| `irodori-ranking-engine` | ランキング定義、入力検証、決定論的observed_score・2種の充足率・confidence・同点・感度分析、結果説明 | Web調査、口コミ要約、出典取得、AIによる自由順位、`products.rank_no`、公開 |
...
8. `observed_score` / `data_coverage` / `weighted_data_coverage` / `confidence` を分離し、未確認項目を自動的に0点にしない(→ `ranking-principles.md`)。
...
| # | 事項 | 現状 |
|---|---|---|
| 1 | 最初に試験する商品 | CYBEX Melio Carbon 2026 / JPを暫定承認。モデル共通型番は未確認、Cinnamon Yellowの商品コード`526000803`はvariantとして分離 |
...
| 6a | 最低weightedデータ充足率 | proposed 0.75 |
