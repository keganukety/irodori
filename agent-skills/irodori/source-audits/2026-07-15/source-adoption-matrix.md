# 情報源採用マトリクス — マイベスト / 価格.com / たまひよ (2026-07-15)
...
| 方式 | マイベスト | 価格.com | たまひよ |
|---|---|---|---|
| A 人がブラウザで読み手動構造化 | PASS(条件付き) | UNKNOWN | PASS(条件付き) |
| B AIブラウザ少数ページ+出典付き短い要約 | PASS(条件付き) | UNKNOWN | PASS(条件付き) |
| C HTML自動取得・構造化 | FAIL | FAIL | FAIL |
| D 定期巡回による更新検知 | FAIL | FAIL | UNKNOWN(不採用) |
| E 公式API・RSS・データ提供 | UNKNOWN | UNKNOWN | UNKNOWN |
| F 客観的商品仕様だけ確認 | PASS(条件付き) | UNKNOWN | PASS(条件付き) |
| G 編集部評価のテーマ・論点構造化 | PASS(条件付き) | NOT_APPLICABLE | PASS(条件付き) |
| H 口コミ1投稿単位の保存 | FAIL | FAIL | FAIL |
| I 件数・肯定/否定傾向のみ保存 | PASS(条件付き) | UNKNOWN | PASS(条件付き) |
| J 他媒体順位の参考メタデータ保存 | FAIL(公開)/UNKNOWN(内部) | UNKNOWN(内部)/FAIL(公開) | PASS(条件付き・内部のみ) |
| K 短い引用の保存 | FAIL | UNKNOWN | UNKNOWN |
| L URL・ページ名・確認日のみ保存 | PASS | PASS | PASS |
...
| 保存項目 | マイベスト | 価格.com | たまひよ |
|---|---|---|---|
| 媒体名・ページ名・URL・確認日 | ◯ | ◯ | ◯ |
| 公開日・更新日 | ◯(確認可能時のみ) | ◯(同左) | ◯(同左) |
| 対象商品・型番・モデル年 | ◯ | ? | ◯ |
| 他媒体内の順位 | ?(内部)/×(公開) | ?(内部)/×(公開) | △(内部のみ) |
| 評価方法の概要 | ◯ | ◯ | ◯ |
| 編集部による評価軸 | △ | — | △ |
| 実測値・測定条件 | ? | — | — |
| 短い評価傾向・肯定/否定テーマ | △ | ? | △ |
...
| 役割 | マイベスト | 価格.com | たまひよ |
|---|---|---|---|
| product_identity_confirmation | 不適(仕様入力に生成AI利用と自己申告) | 保留 | 補助的(公式優先) |
| official_spec_cross_check | 不適 | 保留 | 補助的 |
| third_party_measurement | 保留(Requires Legal Review) | — | — |
| editorial_evaluation | ◯(論点参考・当面はこれのみ) | — | ◯ |
| review_theme_source | 不採用 | 保留 | **◯(第一候補)** |
| buyer_review_source | 不採用 | 保留(承諾取得が前提) | 不採用 |
| external_ranking_metadata | 保留 | 保留 | △(内部のみ) |
| not_adopted | — | **当面not_adopted** | — |

## 5. ランキングとの関係(確定原則の再確認)

3媒体すべてに共通して、以下をIRODORI側の確定原則として適用する。

- 他媒体順位をIRODORI得点へ変換しない / 平均・合算しない / 掲載回数だけで加点しない
- 他媒体の星評価を無条件に共通点へ変換しない
- 広告・アフィリエイト条件を得点へ反映しない
...
- **最初に採用検討する媒体: たまひよ** —
  (1)閲覧・事実参照への規約上の明示的制限が3媒体で最も少ない
