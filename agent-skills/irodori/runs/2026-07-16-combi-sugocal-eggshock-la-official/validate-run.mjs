 ...
 *
 * 実行方法(リポジトリルートから):
 *   node agent-skills/irodori/runs/2026-07-16-combi-sugocal-eggshock-la-official/validate-run.mjs
 */
...
check(
  ...
  identity.site_product_id === "3" && identity.site_product_match_status === "probable",
);
check(
...
// 4) 情報源ポリシー(公式ドメイン限定)
const OFFICIAL_HOSTS = ["www.combi.co.jp", "combi.co.jp"];
...
check(
  ...
  Boolean(manual)
    ...
    && manual.discovery_page_url === "https://www.combi.co.jp/soudan/after/manual_cp/babycar.html"
    && manual.direct_asset_url === manual.url
    && manual.discovered_via_official_page === true,
);
check(
  "口コミ・レビュー本文/投稿者情報を保存していない",
  !/レビュー本文|投稿者名|ハンドル名/.test(JSON.stringify(claims))
    && claims.every((c) => c.claim_class !== "review_aggregate" && c.claim_class !== "user_testimonial"),
);
check(
