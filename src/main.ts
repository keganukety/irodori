...
type QuickFilter = 'all' | 'newborn' | 'lightweight' | 'reversible' | 'train' | 'compactCar' | 'travel';
...
async function renderPublicPage() {
  ...
  const strollerCategories = getStrollerCategoryValues(categoryCounts);
  const categoryFilteredProducts = allProducts.filter((product) => strollerCategories.has(normalizeCategory(product.category)));
  console.log('ベビーカー絞り込み後の件数', categoryFilteredProducts.length);

  strollerProducts = dedupeProductsById(categoryFilteredProducts);
  console.log('重複除去後の件数', strollerProducts.length);
    ...
    <main class="storefront-shell">
      <section class="hero-panel">
        <div class="hero-inner">
          <h1 class="product-list-title">ベビーカー</h1>
          <p class="hero-copy">月齢・重さ・移動スタイルから、暮らしに合う一台を探せます。</p>
      ...
      <div class="content-shell">
        <div class="quick-filters" aria-label="クイックフィルター">
          ...
          ${renderQuickFilter('newborn', '新生児OK')}
          ${renderQuickFilter('lightweight', '軽量')}
          ${renderQuickFilter('reversible', '両対面')}
          ${renderQuickFilter('train', '電車移動')}
          ${renderQuickFilter('compactCar', '軽自動車')}
          ${renderQuickFilter('travel', '旅行')}
        </div>
  ...
  return `
    <details class="filter-sidebar">
      ...
      <div class="filter-panel">
        ...
        ${renderFilterGroup('タイプ', [['type:A型', 'A型'], ['type:B型', 'B型'], ['type:AB型', 'AB型'], ['type:三輪', '三輪'], ['type:コンパクト', 'コンパクト'], ['type:両対面', '両対面']])}
        ${renderFilterGroup('シーンで探す', [['scene:電車移動', '電車移動'], ['scene:軽自動車', '軽自動車'], ['scene:マンション', 'マンション'], ['scene:ワンオペ', 'ワンオペ'], ['scene:旅行', '旅行'], ['scene:新生児', '新生児'], ['scene:飛行機', '飛行機']])}
        ...
        ${renderFilterGroup('カラー', [['color:black', 'ブラック系'], ['color:beige', 'ベージュ系'], ['color:gray', 'グレー系'], ['color:navy', 'ネイビー系']])}
  ...
  return `
    <details class="filter-group filter-accordion category-navigation" open>
      ...
      <nav class="filter-options" aria-label="商品カテゴリ">${categories.map((category) => {
        const count = allProducts.filter((product) => normalizeCategory(product.category) === category).length;
        const isCurrent = category === 'ベビーカー';
        const href = isCurrent ? '/products.html' : `/products.html?category=${encodeURIComponent(category)}`;
        return `<a class="${isCurrent ? 'is-current' : ''}" href="${escapeAttr(href)}" ${isCurrent ? 'aria-current="page"' : ''}><span>${escapeText(category)}</span><small>${count}</small></a>`;
      }).join('')}</nav>
    </details>`;
}
...
function matchesQuickFilter(product: Product) {
  ...
  if (activeQuickFilter === 'newborn') return /新生児|生後\s*1|1カ月|1ヶ月/.test(haystack);
  if (activeQuickFilter === 'lightweight') return weight > 0 && weight < 5;
  if (activeQuickFilter === 'reversible') return /両対面/.test(haystack);
  if (activeQuickFilter === 'train') return /軽量|コンパクト|電車|ワンタッチ|折りたたみ/.test(haystack);
  if (activeQuickFilter === 'compactCar') return /コンパクト|軽自動車|折りたたみ/.test(haystack);
  if (activeQuickFilter === 'travel') return /旅行|軽量|コンパクト|機内|トラベル/.test(haystack);
  return true;
}
